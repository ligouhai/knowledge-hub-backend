...
本地全新初始化：删除主机目录 `volumes/postgres` 后 `docker compose up -d`，容器会执行 `init-scripts/postgresql/init.sql`（不是仓库根目录的 `postgresql/init.sql`），从而创建 `kh_document` 与 `kh_document_review`。
