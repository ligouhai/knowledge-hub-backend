...
export class GraphBuildService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    // 默认与 docker-compose NEO4J_AUTH=neo4j/12345678 对齐
    const password = this.config.get<string>('NEO4J_PASSWORD', '12345678');
    this.driver = neo4j.driver(uri, neo4j.auth.basic(user, password));
    try {
      await this.driver.verifyConnectivity();
      this.logger.log(`Neo4j 已连接：${uri}（user=${user}）`);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.logger.warn(
        `Neo4j 不可用，KG 写入将跳过：${message}（uri=${uri}, user=${user}）。请核对 NEO4J_PASSWORD 是否与 docker-compose 中 NEO4J_AUTH 一致`,
      );
